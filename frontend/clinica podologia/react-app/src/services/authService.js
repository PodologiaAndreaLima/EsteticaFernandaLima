...
export const AuthService = {
  ...
  // Envia um email com código OTP para o funcionário
  ...
  // Função para redefinir senha com código OTP
  resetPassword: async (email, code, newPassword) => {
    try {
      ...
      return {
        ...
        error:
          extractApiErrorMessage(error) ||
          getFriendlyPasswordError(error, "Erro ao redefinir senha"),
      };
    }
  },

  // Função para admin alterar senha de um funcionário
  // Requer @PreAuthorize("ROLE_ADMIN") no backend
  changeEmployeePassword: async (funcionarioId, newPassword) => {
    try {
      if (!funcionarioId) {
        return {
          success: false,
      ...
      const response = await api.post(
        `/funcionarios/${funcionarioId}/alterar-senha`,
        {
      ...
      return {
        success: true,
        message: "Senha do funcionário alterada com sucesso!",
        data: response.data,
      };
    } catch (error) {
      return {
        ...
        error:
          ...
          getFriendlyPasswordError(
            error,
            "Erro ao alterar senha do funcionário",
          ),
      };
    }
  },
